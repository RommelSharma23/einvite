...
/**
 * Background job to check pending domain verifications
...
export async function checkPendingVerifications() {
  try {
    ...
    // Get pending verifications that haven't reached max attempts
    const { data: pendingDomains, error } = await supabase
      ...
      .lt('verification_attempts', 5) // Less than max attempts
      .gt('expires_at', new Date().toISOString()) // Not expired

    if (error || !pendingDomains?.length) {
      console.log('No pending domain verifications to check')
      return
    }

    console.log(`Checking ${pendingDomains.length} pending domain verifications...`)

    for (const domain of pendingDomains) {
      try {
        // Verify domain
        const verificationResult = await verifyDomainOwnershipServer(
          domain.custom_domain,
          domain.verification_token
        )

        // Update status
        const newStatus = verificationResult.success ? 'verified' : 'pending'
        
        await supabase.rpc('update_domain_verification_status', {
          domain_id_param: domain.id,
          new_status: newStatus,
          error_msg: verificationResult.error || null,
          dns_records: verificationResult.records || []
        })

        // Update cache if verified
        if (verificationResult.success) {
          const project = domain.wedding_projects as unknown as { subdomain: string; is_published: boolean }
          if (project.subdomain && project.is_published) {
            redirectCache.setRedirect(project.subdomain, domain.custom_domain, true)
          }
          
          console.log(`✅ Background verification successful: ${domain.custom_domain}`)
        }

      } catch (error) {
        console.error(`Error checking domain ${domain.custom_domain}:`, error)
      }
    }

  } catch (error) {
    console.error('Error in background domain verification:', error)
  }
}